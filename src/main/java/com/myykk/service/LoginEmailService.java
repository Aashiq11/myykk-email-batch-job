...
public class LoginEmailService {
	...
	public void sendLoginEmailJob() {

		try {
			List<EmailDTO> emailList = notificationRepository.getEmailListForFuncId("L");
				...
				if (userDetails != null) {
			       ...
			       // String imagepath = com.myykk.constants.YKKConstants.EMAIL_BANNER_PATH;
			        ...
			        if ("1".equals( emailDTO.getLangflg())) {
			            ...
			            emailBean.setHtmlTemplate(vmTemplatePath + "/login.vm");
			            salutation = "Dear";
			        } else {
			            emailBean
			                    .setEmailSubjet("Contrasena para la pagina de myYKK ");
			            emailBean.setHtmlTemplate(vmTemplatePath + "/login_es.vm");
			            salutation = "Estimado";
			        }
